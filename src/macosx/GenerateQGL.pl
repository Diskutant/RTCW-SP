...
open(CPP2, "cpp /tmp/input-$$.h|") || die "$!";


print "/**** This file is autogenerated.  Run GenerateQGL.pl to update it ****/\n\n";

print "#import <OpenGL/gltypes.h>\n";
print "#import <stdio.h>\n";
...
print "extern inline GLenum glGetError(void);\n";
...
while (<CPP2>) {
	...
	# Get the type and shit
	($type, $rest) = m/(.+)\s+(gl.*)/;
	# Get more shit
	($name, $argString) = ($rest =~ m/(\w+).*\s*\((.*)\)/);
	
	print "extern $type $name($argString);\n";
}
...
while (<CPP>) {
    ...
    ($name, $argString) = ($rest =~ m/(\w+).*\s*\((.*)\)/);
    $isVoid = ($type =~ m/void/ || $type =~ m/GLvoid/ && $_ !~ m/GLvoid[\t ]+\*/);
...
#    print "argCount=$#args\n";
    ...
    # Parse the function argument list into two arrays, one of types and one of argument names
    if ($argString =~ m/^void$/) {
        @args = ();
    } else {
...
#        print "argName='$argName'\n";
    ...
    if ($#argTypes >= 0) {
	    ...
	    if (/^GLenum$/ || /^GLuint$/ || /^GLbitfield$/ || /^GLuint64$/) {

                print "$argName=%lu";

	    } elsif (/^GLsizei$/ || /^GLint$/ || /^GLsizeiptr$/ || /^GLsizeiptrARB$/ || /^GLintptr$/ || /^GLintptrARB$/) {
